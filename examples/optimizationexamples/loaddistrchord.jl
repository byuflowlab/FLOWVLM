# Induced drag minimization respect to chord length distribution using Bertin's
# wing as the baseline, subject to a specific lift coefficient.
...
save_path = joinpath(modulepath, "../../temps/opt_bwchord01/")  # Output folder
run_name = "opt_bwchord"
...
fobj_nin = nc               # Number of input variables on objective (same than x0)
...
x0 = clen                   # Initial guess
lb = 0.01*ones(fobj_nin)    # Lower bounds
ub = 2.0*ones(fobj_nin)     # Upper bounds
