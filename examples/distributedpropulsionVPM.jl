...
myvpm_path = "/home/user/Dropbox/FLOWResearch/MyCodes/MyVPM/"
...
function distributedpropulsionVPM(; save_path=module_path*"/../temps/distpropVPM03/",
  ...
  dt = 1/(steps_per_rev*RPM/60)  # (s) time step
  solver = "ExaFMM"                 # VPM solver
  omega_fun(t) = omega              # Angular velocity function
  ...
  # --------------- INITIALIZES VPM -------------------------------
  nsteps_save = 1                     # Number of steps between outputing vtks
  max_particles = 18000               # Particles it keep in the field
  n_steps_shedding = 1                # Time steps in between sheddings
  p_field = vpm.ParticleField(Int64(ceil(max_particles*1.25)),
                                                Vinf, nothing, solver)

  # --------------- SETS UP RUNTIME ROUTINE ----------------------------------
  del_n = 1                           # Particle that will delete next
  forward_del = true                  # Flag for deleting forward or backward

  # Delete function
  function delete_particles(PFIELD, max_p)
    while PFIELD.np>max_p
      vpm.delparticle(PFIELD, del_n)
      del_n = forward_del ? del_n+1 : del_n-1

      if forward_del && del_n==max_p+1
        del_n = max_p
        forward_del = false
      elseif false==forward_del && del_n==0
        del_n = 1
        forward_del = true
      end
    end
  ...
  function runtime_function(PFIELD, T, DT)
    ...
    # Deletes particles
    if PFIELD.nt%n_steps_shedding==0
      delete_particles(PFIELD, max_particles)
    end
  ...
  # --------------- RUN THE VPM ----------------------------------------------
  vpm.run_vpm!(p_field, dt, nsteps; save_path=save_path, run_name=save_name,
                    runtime_function=runtime_function, solver_method=solver,
                    nsteps_save=nsteps_save,
                  save_code=joinpath(module_path,"distributedpropulsionVPM.jl"),
                    prompt=prompt)
  ...
  if save_path!=nothing
    ...
    if paraview
      ...
      strn = strn * save_name * "_pfield...vtk;"
...
an infinite vortex starts. `dt` indicates the length of the interval of time of
vortex shedding the particles will represent."
function VLM2VPM(system, pfield::vpm.ParticleField, dt::Float64; t::Float64=0.0,
                  check=true, verbose=false, tol=1e-6)
  ...
  for i in 1:m  # Iterates over lattices
    ...
    prev_Ap, prev_A, prev_B, prev_Bp, prev_CP, prev_infDA, prev_infDB, prev_Gamma = prev_HS

        aux = vcat(Ap, A, B, Bp, CP, infDA, infDB, Gamma)
        if true in [isnan(elem) for elem in aux]
          error("Logic error! $aux")
        end
    ...
    if i==1
      ...
      # sigma = V*dt/2                            # Vortex blob radius
      #
      # add_particle(pfield, X, gamma, dt, V, infD, sigma)

    # ----------- Case of wing tip on discontinuous wing --------
    elseif norm(Ap - prev_Bp) / norm(Bp - Ap) > tol
      if verbose; println("Wing tip found at i=$i. $Ap!=$prev_Bp"); end;
    ...
    # ----------- Case of contiguous horseshoes -----------------
      ...
      if i==m
        ...
        sigma = V*dt/2                            # Vortex blob radiuss
...
function add_particle(pfield::vpm.ParticleField, X::Array{Float64, 1},
  ...
  particle = vcat(X, Gamma, 10.0*sigma)
